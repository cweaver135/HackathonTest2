...
def main():
    ...
    while user_char != 1 or user_char != 2 or user_char != 3 or user_char != 4:
        user_char = int(input("Please choose a number between 1 and 4: "))

    if user_char==1 or user_char==2:
        
        if user_char== 1:
            user=Person("fancy dagger",10000,3,100,"Princess",0,64)
            computer=Person("club",4,7,60,"Hobbit",0,42)
                
        if user_char == 2:
            computer=Person("fancy dagger",10000,3,100,"Princess",0,64)
...
def epic_story(user):
    ...
    def task1(user, computer):
        ...
        print(computer.get_species() + ": Uhhh... " + time.sleep(2), "I didn't see anything?")
        ...
        if help == "y":
            ...
            if user.get_species() == "Witch":
                ...
                if answer == "Hg":
                    ...
                    print("You were mauled to death by the wolf...", time.sleep(2), "GAME OVER.")
                    user.set_health(0)
            if user.get_species() == "Elf":
                answer = input("What is the derivative of cos(x)?")
                if answer == "-sin(x)":
                    print("You shoot the wolf in the heart with an arrow.")
            print("Tiny Crimson Commuting Cowl: ...", time.sleep(3), "You killed him...", time.sleep(1), "THANK YOU!!! As a thank you, here's my cape.")
            user.set_friends("Tiny Crimson Commuting Cowl")
            print("You made a new friend!")
            print("Friends: " + user.get_friends())
        elif help == "n":
            print("You were munched to death by the Wolf...", time.sleep(2), "GAME OVER.")
            user.set_health(0)
        else:
    ...
    def task3(user, computer):
        print("You see a figure approach you....", time.sleep(3))
        ...
        print("The Witch leads you through the forest. 𖠰↟𖠰𖠰")
        ...
        print(computer.get_species(), ": Alright, I'll start in the kitchen.", time.sleep(5), "... ", user.get_species(), " come here")
        ...
        while answer not in "yn":
            if answer == "y":
                    ...
                    if answer == "y":
                        ...
                        if answer1 == "quartic" or "Quartic":
                            if user.get_species()=="Witch":
                                ...
                                print("The two children stare blankly for a moment", time.sleep(1))
                ...
                if answer == "n":
                    ...
                    print("Witch: Well thank you for a job well done, here is your payment!", time.sleep(2))
                    new_health = user.get_health = 100
                    user.set_health(new_health)
                    print("Witch: I've healed you to full health my friends. Thank you.")
                    user.get_friends("Witch")
    ...
    def task4(user, computer):
        # rumpelstilskin
        print("↟𖠰↟𖠰 You begin another long walk within the forest. 𖠰↟𖠰𖠰")
        print("You stumble across a large cobblestone bridge which towers unnecessarily above a small stream, it seems almost useless to even go over it")
        print ("???: IT IS I...")
        time.sleep(3)
        print("R: ...Rumplestiltzkin!")
        print("R: Now that you've found me, you must answer my riddles 3 or you may never escape! >:D")
        print("R: First...")
        time.sleep(3)
        riddleoneanswer = input("What feels like green paint, smells like green paint, but isn't green paint? ")
        if riddleoneanswer == ("paint"):
            print("R: WOAH you've answered my riddle one... and got it correct, i'm honestly stunned. Just as a reward you will answer just one more riddle... MY RIDDLE ONE!")
        else:
            print("R: heh heh, just as i suspected you got it UNBELIEVABLY incorrect. The only way to save you now is to answer one final riddle. MY RIDDLE ONE!")
        riddletwoanswer = input("R: What is my name heh heh? ")
        if riddletwoanswer == ("rumplestiltzkin") or ("Rumplestiltzkin"):
            print("....")
            time.sleep(3)
            print("Rumplestiltzkin: I have absolutely no clue how you knew that...")
            print("Rumplestiltzkin: well a deal is a deal and I shall set you free, and with a reward I suppose. And of course access to my bridge!")
            print(user.get_species, ": oh... thanks i guess.")
            money = user.get_money() + 10
            user.set_money(money)
            user.set_friend("Rumplestiltzkin")
        else:
            print("Rumplestiltzkin: L! haha you absolute bafoon that is just not my name. now you must pay the price of prices! 3 pieces of gold in payment for my riddles 3")
